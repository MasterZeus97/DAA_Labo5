...
fun AppContact(application: ContactsApplication, contactsViewModel : ContactsViewModel = viewModel(factory= ContactsViewModelFactory(application))) {
    ...
    { padding ->
        Column(modifier = Modifier.padding(padding)) {
            ...
            if(selectedContact == null){
                ...
                if(b) {
                    ...
                    ScreenContactEditor(title = "New contact", contact = selectedContact!!, delete = {}, back = {}, validate = {}, validateText = "CREATE")
                }else{
        ...
        /*when(b){
            0-> ScreenContactList(contacts) { selectedContact ->
                    Toast.makeText(context, "TODO - Edition de ${selectedContact.firstname} ${selectedContact.name}", Toast.LENGTH_SHORT).show()
                }
            1 -> ScreenContactEditor(contactsViewModel, "bla")
            else -> ScreenContactList(contacts){ selectedContact ->
                Toast.makeText(context, "TODO - Edition de ${selectedContact.firstname} ${selectedContact.name}", Toast.LENGTH_SHORT).show()
            }
        }*/
    /*if(b){
            ...
            ScreenContactEditor(contactsViewModel)
        }*/
        }
    }
